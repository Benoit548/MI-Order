...
import com.example.mi_order.repository.OrderRepository.Singleton.databaseRefOrder
import com.example.mi_order.repository.OrderRepository.Singleton.orderList
...
class OrderRepository {
    ...
    object Singleton {
        ...
        val databaseRefOrder = FirebaseDatabase.getInstance("https://mi-order-ad764-default-rtdb.europe-west1.firebasedatabase.app/").getReference("Orders")
        ...
        databaseRefOrder.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) { //Il recupere une liste d'objet qui n'est pas encore un FoodModel
                ...
                for (ds in snapshot.children) {//ds c'est le nom générique de DataSnapshot
                    ...
                    val order = ds.getValue(OrderModel::class.java)
    ...
    //Insérer un order en BDD
    fun insertOrder(order: OrderModel) = databaseRefOrder.child(order.id).setValue(order)

    // Supprimer
    fun deleteOrder(order: OrderModel) = databaseRefOrder.child(order.id).removeValue()
}
